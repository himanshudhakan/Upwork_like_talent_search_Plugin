				...
				<div class="hdpa-filter-field">
					...
					<select class="hdpa-select" id="hdpa_filter_skills" name="hdpa_filter_skills" multiple>
						<?php foreach ( $skills as $skill_id => $skill ) { ?>
							<option value="<?php echo esc_attr( $skill_id ); ?>"><?php echo esc_html( $skill ); ?></option>
						<?php } ?>
					</select>
				</div>
			</div>
				...
				<div class="hdpa-filter-field">
					...
					<select class="hdpa-select" id="hdpa_filter_education" name="hdpa_filter_education" multiple>
						<?php foreach ( $education as $education_id => $education ) { ?>
							<option value="<?php echo esc_attr( $education_id ); ?>"><?php echo esc_html( $education ); ?></option>
						<?php } ?>
					</select>
				</div>
			</div>
			...
			<div class="hdpa-filter-col hdpa-col-two">
				<div class="hdpa-filter-field">
					...
					<div class="hdpa-filter-age-wrap">
						...
						<p class="hdpa-filter-age-text"><span class="hdpa_filter_age_from_text">1</span> <?php esc_html_e( 'To', 'hdpa' ); ?> <span class="hdpa_filter_age_to_text"></span></p>
					</div>
				</div>
			</div>
			<div class="hdpa-filter-col hdpa-col-two">
				<div class="hdpa-filter-field hdpa-filter-field-ratings">
					...
					<div class="hdpa-filter-field-content">
						<div class="hdpa-rating">
							<?php for ( $i = 5; $i > 0; $i-- ) { ?>
								<?php $rat_num = $i; ?>
								<input type="radio" id="star<?php echo esc_attr( $rat_num ); ?>" name="hdpa_filter_ratings" value="<?php echo esc_attr( $rat_num ); ?>" /><label for="star<?php echo esc_attr( $rat_num ); ?>"><i class="rating__icon rating__icon--star dashicons dashicons-star-filled"></i></label>
							<?php } ?>
						</div>
					</div>
				</div>
